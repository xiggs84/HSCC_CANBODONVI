...
class TaiSanResourceIT {
    ...
    void getAllTaiSans() throws Exception {
        ...
        restTaiSanMockMvc
            ...
            .andExpect(jsonPath("$.[*].idTaiSan").value(hasItem(taiSan.getIdTaiSan().intValue())))
            ...
            .andExpect(jsonPath("$.[*].thongTinTs").value(hasItem(DEFAULT_THONG_TIN_TS.toString())))
    ...
    void getTaiSan() throws Exception {
        ...
        // Get the taiSan
        restTaiSanMockMvc
            .perform(get(ENTITY_API_URL_ID, taiSan.getIdTaiSan()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.idTaiSan").value(taiSan.getIdTaiSan().intValue()))
            .andExpect(jsonPath("$.tenTaiSan").value(DEFAULT_TEN_TAI_SAN))
            .andExpect(jsonPath("$.trangThai").value(DEFAULT_TRANG_THAI.intValue()))
            .andExpect(jsonPath("$.thongTinTs").value(DEFAULT_THONG_TIN_TS.toString()))
    ...
    void getTaiSansByIdFiltering() throws Exception {
        ...
        Long id = taiSan.getIdTaiSan();

        defaultTaiSanFiltering("idTaiSan.equals=" + id, "idTaiSan.notEquals=" + id);

        defaultTaiSanFiltering("idTaiSan.greaterThanOrEqual=" + id, "idTaiSan.greaterThan=" + id);

        defaultTaiSanFiltering("idTaiSan.lessThanOrEqual=" + id, "idTaiSan.lessThan=" + id);
    }
    ...
    @Test
    @Transactional
    void getAllTaiSansByDanhMucLoaiTaiSanIsEqualToSomething() throws Exception {
        ...
        em.flush();
        taiSan.setDanhMucLoaiTaiSan(danhMucLoaiTaiSan);
        taiSanRepository.saveAndFlush(taiSan);
        Long danhMucLoaiTaiSanId = danhMucLoaiTaiSan.getIdLoaiTs();
        // Get all the taiSanList where danhMucLoaiTaiSan equals to danhMucLoaiTaiSanId
        defaultTaiSanShouldBeFound("danhMucLoaiTaiSanId.equals=" + danhMucLoaiTaiSanId);

        // Get all the taiSanList where danhMucLoaiTaiSan equals to (danhMucLoaiTaiSanId + 1)
        defaultTaiSanShouldNotBeFound("danhMucLoaiTaiSanId.equals=" + (danhMucLoaiTaiSanId + 1));
    }

    @Test
    @Transactional
    void getAllTaiSansByTinhTrangTaiSanIsEqualToSomething() throws Exception {
        TinhTrangTaiSan tinhTrangTaiSan;
        if (TestUtil.findAll(em, TinhTrangTaiSan.class).isEmpty()) {
            taiSanRepository.saveAndFlush(taiSan);
            tinhTrangTaiSan = TinhTrangTaiSanResourceIT.createEntity();
        } else {
            tinhTrangTaiSan = TestUtil.findAll(em, TinhTrangTaiSan.class).get(0);
        }
        em.persist(tinhTrangTaiSan);
        em.flush();
        taiSan.setTinhTrangTaiSan(tinhTrangTaiSan);
        taiSanRepository.saveAndFlush(taiSan);
        Long tinhTrangTaiSanId = tinhTrangTaiSan.getIdTinhTrang();
        // Get all the taiSanList where tinhTrangTaiSan equals to tinhTrangTaiSanId
        defaultTaiSanShouldBeFound("tinhTrangTaiSanId.equals=" + tinhTrangTaiSanId);

        // Get all the taiSanList where tinhTrangTaiSan equals to (tinhTrangTaiSanId + 1)
        defaultTaiSanShouldNotBeFound("tinhTrangTaiSanId.equals=" + (tinhTrangTaiSanId + 1));
    }
    ...
    private void defaultTaiSanShouldBeFound(String filter) throws Exception {
        restTaiSanMockMvc
            .perform(get(ENTITY_API_URL + "?sort=idTaiSan,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].idTaiSan").value(hasItem(taiSan.getIdTaiSan().intValue())))
            .andExpect(jsonPath("$.[*].tenTaiSan").value(hasItem(DEFAULT_TEN_TAI_SAN)))
        ...
        restTaiSanMockMvc
            .perform(get(ENTITY_API_URL + "/count?sort=idTaiSan,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    void putExistingTaiSan() throws Exception {
        ...
        // Update the taiSan
        TaiSan updatedTaiSan = taiSanRepository.findById(taiSan.getIdTaiSan()).orElseThrow();
        ...
        restTaiSanMockMvc
            .perform(
                put(ENTITY_API_URL_ID, taiSanDTO.getIdTaiSan())
    ...
    void putNonExistingTaiSan() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        taiSan.setIdTaiSan(longCount.incrementAndGet());
    ...
    void partialUpdateTaiSanWithPatch() throws Exception {
        ...
        partialUpdatedTaiSan.setIdTaiSan(taiSan.getIdTaiSan());
        ...
        restTaiSanMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedTaiSan.getIdTaiSan())
    ...
    void patchNonExistingTaiSan() throws Exception {
        ...
        restTaiSanMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, taiSanDTO.getIdTaiSan())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(taiSanDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteTaiSan() throws Exception {
        ...
        // Delete the taiSan
        restTaiSanMockMvc
            .perform(delete(ENTITY_API_URL_ID, taiSan.getIdTaiSan()).accept(MediaType.APPLICATION_JSON))
    ...
    protected TaiSan getPersistedTaiSan(TaiSan taiSan) {
        return taiSanRepository.findById(taiSan.getIdTaiSan()).orElseThrow();
    }
