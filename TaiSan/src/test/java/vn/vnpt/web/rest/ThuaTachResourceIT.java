...
class ThuaTachResourceIT {
    ...
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{idThuaTach}";
     ...
     */
    public static ThuaTach createEntity() {
        return new ThuaTach().thongTinThuaTach(DEFAULT_THONG_TIN_THUA_TACH).trangThai(DEFAULT_TRANG_THAI);
    }
     ...
     * if they test an entity which requires the current entity.
     */
    public static ThuaTach createUpdatedEntity() {
        return new ThuaTach().thongTinThuaTach(UPDATED_THONG_TIN_THUA_TACH).trangThai(UPDATED_TRANG_THAI);
    }

    @BeforeEach
    public void initTest() {
        thuaTach = createEntity();
    }
    ...
    void getAllThuaTaches() throws Exception {
        ...
        restThuaTachMockMvc
            .perform(get(ENTITY_API_URL + "?sort=idThuaTach,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].idThuaTach").value(hasItem(thuaTach.getIdThuaTach().intValue())))
    ...
    void getThuaTach() throws Exception {
        ...
        // Get the thuaTach
        restThuaTachMockMvc
            .perform(get(ENTITY_API_URL_ID, thuaTach.getIdThuaTach()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.idThuaTach").value(thuaTach.getIdThuaTach().intValue()))
    ...
    void putExistingThuaTach() throws Exception {
        ...
        // Update the thuaTach
        ThuaTach updatedThuaTach = thuaTachRepository.findById(thuaTach.getIdThuaTach()).orElseThrow();
        // Disconnect from session so that the updates on updatedThuaTach are not directly saved in db
        em.detach(updatedThuaTach);
        updatedThuaTach.thongTinThuaTach(UPDATED_THONG_TIN_THUA_TACH).trangThai(UPDATED_TRANG_THAI);
        ThuaTachDTO thuaTachDTO = thuaTachMapper.toDto(updatedThuaTach);

        restThuaTachMockMvc
            .perform(
                put(ENTITY_API_URL_ID, thuaTachDTO.getIdThuaTach())
    ...
    void putNonExistingThuaTach() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        thuaTach.setIdThuaTach(longCount.incrementAndGet());
    ...
    void partialUpdateThuaTachWithPatch() throws Exception {
        ...
        partialUpdatedThuaTach.setIdThuaTach(thuaTach.getIdThuaTach());

        partialUpdatedThuaTach.trangThai(UPDATED_TRANG_THAI);

        restThuaTachMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedThuaTach.getIdThuaTach())
    ...
    void patchNonExistingThuaTach() throws Exception {
        ...
        restThuaTachMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, thuaTachDTO.getIdThuaTach())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(thuaTachDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void deleteThuaTach() throws Exception {
        ...
        // Delete the thuaTach
        restThuaTachMockMvc
            .perform(delete(ENTITY_API_URL_ID, thuaTach.getIdThuaTach()).accept(MediaType.APPLICATION_JSON))
    ...
    protected ThuaTach getPersistedThuaTach(ThuaTach thuaTach) {
        return thuaTachRepository.findById(thuaTach.getIdThuaTach()).orElseThrow();
    }
