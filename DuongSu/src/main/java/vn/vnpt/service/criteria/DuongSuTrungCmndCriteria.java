...
/**
 * Criteria class for the {@link vn.vnpt.domain.DuongSuTrungCmnd} entity. This class is used
 * in {@link vn.vnpt.web.rest.DuongSuTrungCmndResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /duong-su-trung-cmnds?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
@ParameterObject
@SuppressWarnings("common-java:DuplicatedBlocks")
public class DuongSuTrungCmndCriteria implements Serializable, Criteria {
    ...
    private LongFilter idDuongSuMin;

    private LongFilter idMasterMin;

    private LongFilter idDuongSuMax;

    private LongFilter idMasterMax;

    private LongFilter duongSuId;

    private Boolean distinct;

    public DuongSuTrungCmndCriteria() {}

    public DuongSuTrungCmndCriteria(DuongSuTrungCmndCriteria other) {
        ...
        this.idDuongSuMin = other.optionalIdDuongSuMin().map(LongFilter::copy).orElse(null);
        this.idMasterMin = other.optionalIdMasterMin().map(LongFilter::copy).orElse(null);
        this.idDuongSuMax = other.optionalIdDuongSuMax().map(LongFilter::copy).orElse(null);
        this.idMasterMax = other.optionalIdMasterMax().map(LongFilter::copy).orElse(null);
        this.duongSuId = other.optionalDuongSuId().map(LongFilter::copy).orElse(null);
        this.distinct = other.distinct;
    }

    @Override
    public DuongSuTrungCmndCriteria copy() {
        return new DuongSuTrungCmndCriteria(this);
    }
    ...
    public LongFilter getIdDuongSuMin() {
        return idDuongSuMin;
    }

    public Optional<LongFilter> optionalIdDuongSuMin() {
        return Optional.ofNullable(idDuongSuMin);
    }

    public LongFilter idDuongSuMin() {
        if (idDuongSuMin == null) {
            setIdDuongSuMin(new LongFilter());
        }
        return idDuongSuMin;
    }

    public void setIdDuongSuMin(LongFilter idDuongSuMin) {
        this.idDuongSuMin = idDuongSuMin;
    }

    public LongFilter getIdMasterMin() {
        return idMasterMin;
    }

    public Optional<LongFilter> optionalIdMasterMin() {
        return Optional.ofNullable(idMasterMin);
    }

    public LongFilter idMasterMin() {
        if (idMasterMin == null) {
            setIdMasterMin(new LongFilter());
        }
        return idMasterMin;
    }

    public void setIdMasterMin(LongFilter idMasterMin) {
        this.idMasterMin = idMasterMin;
    }

    public LongFilter getIdDuongSuMax() {
        return idDuongSuMax;
    }

    public Optional<LongFilter> optionalIdDuongSuMax() {
        return Optional.ofNullable(idDuongSuMax);
    }

    public LongFilter idDuongSuMax() {
        if (idDuongSuMax == null) {
            setIdDuongSuMax(new LongFilter());
        }
        return idDuongSuMax;
    }

    public void setIdDuongSuMax(LongFilter idDuongSuMax) {
        this.idDuongSuMax = idDuongSuMax;
    }

    public LongFilter getIdMasterMax() {
        return idMasterMax;
    }

    public Optional<LongFilter> optionalIdMasterMax() {
        return Optional.ofNullable(idMasterMax);
    }

    public LongFilter idMasterMax() {
        if (idMasterMax == null) {
            setIdMasterMax(new LongFilter());
        }
        return idMasterMax;
    }

    public void setIdMasterMax(LongFilter idMasterMax) {
    ...
    public boolean equals(Object o) {
        ...
        final DuongSuTrungCmndCriteria that = (DuongSuTrungCmndCriteria) o;
        return (
            ...
            Objects.equals(idDuongSuMin, that.idDuongSuMin) &&
            Objects.equals(idMasterMin, that.idMasterMin) &&
            Objects.equals(idDuongSuMax, that.idDuongSuMax) &&
            Objects.equals(idMasterMax, that.idMasterMax) &&
    ...
    public String toString() {
        return "DuongSuTrungCmndCriteria{" +
            ...
            optionalIdDuongSuMin().map(f -> "idDuongSuMin=" + f + ", ").orElse("") +
            optionalIdMasterMin().map(f -> "idMasterMin=" + f + ", ").orElse("") +
            optionalIdDuongSuMax().map(f -> "idDuongSuMax=" + f + ", ").orElse("") +
            optionalIdMasterMax().map(f -> "idMasterMax=" + f + ", ").orElse("") +
