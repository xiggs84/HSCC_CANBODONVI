...
import vn.vnpt.service.DanhSachDuongSuQueryService;
...
public class DanhSachDuongSuResource {
    ...
    private final DanhSachDuongSuQueryService danhSachDuongSuQueryService;

    public DanhSachDuongSuResource(
        DanhSachDuongSuService danhSachDuongSuService,
        DanhSachDuongSuRepository danhSachDuongSuRepository,
        DanhSachDuongSuQueryService danhSachDuongSuQueryService
    ) {
        this.danhSachDuongSuService = danhSachDuongSuService;
        this.danhSachDuongSuRepository = danhSachDuongSuRepository;
        this.danhSachDuongSuQueryService = danhSachDuongSuQueryService;
    }
     ...
     */
    @PostMapping("")
    public ResponseEntity<DanhSachDuongSuDTO> createDanhSachDuongSu(@Valid @RequestBody DanhSachDuongSuDTO danhSachDuongSuDTO)
    ...
    public ResponseEntity<DanhSachDuongSuDTO> updateDanhSachDuongSu(
        ...
        @Valid @RequestBody DanhSachDuongSuDTO danhSachDuongSuDTO
    ) throws URISyntaxException {
    ...
    public ResponseEntity<DanhSachDuongSuDTO> partialUpdateDanhSachDuongSu(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody DanhSachDuongSuDTO danhSachDuongSuDTO
    ) throws URISyntaxException {
    ...
    public ResponseEntity<List<DanhSachDuongSuDTO>> getAllDanhSachDuongSus(
        DanhSachDuongSuCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get DanhSachDuongSus by criteria: {}", criteria);

        Page<DanhSachDuongSuDTO> page = danhSachDuongSuQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /danh-sach-duong-sus/count} : count all the danhSachDuongSus.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDanhSachDuongSus(DanhSachDuongSuCriteria criteria) {
        LOG.debug("REST request to count DanhSachDuongSus by criteria: {}", criteria);
        return ResponseEntity.ok().body(danhSachDuongSuQueryService.countByCriteria(criteria));
    }
