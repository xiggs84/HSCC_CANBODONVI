...
import vn.vnpt.service.DmXaTmpQueryService;
...
public class DmXaTmpResource {
    ...
    private final DmXaTmpQueryService dmXaTmpQueryService;

    public DmXaTmpResource(DmXaTmpService dmXaTmpService, DmXaTmpRepository dmXaTmpRepository, DmXaTmpQueryService dmXaTmpQueryService) {
        this.dmXaTmpService = dmXaTmpService;
        this.dmXaTmpRepository = dmXaTmpRepository;
        this.dmXaTmpQueryService = dmXaTmpQueryService;
    }
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<DmXaTmpDTO>> getAllDmXaTmps(
        DmXaTmpCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get DmXaTmps by criteria: {}", criteria);

        Page<DmXaTmpDTO> page = dmXaTmpQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /dm-xa-tmps/count} : count all the dmXaTmps.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countDmXaTmps(DmXaTmpCriteria criteria) {
        LOG.debug("REST request to count DmXaTmps by criteria: {}", criteria);
        return ResponseEntity.ok().body(dmXaTmpQueryService.countByCriteria(criteria));
    }
